...
local function printTableRecursive(t, printedTables, indent)
    ...
    if printedTables[t] then
        -- Table already printed, avoid printing it again
...
local function generateVariableName(variable_names)
	...
	repeat
        ...
        if lkup_vars[name] then
            ...
            break -- Exit the loop if the name is available
        end
    until false
...
MinifyNodeFuncs = {
	...
	[NodeVariant.ExprLiteral] = function(node,variable_names)
        ...
        if type == "s" then
            local s = ""
            for i = 1, #value do
                s = s .. string.format("\\x%02X", string.byte(value, i))
            end
            return "\"" .. s .. "\""
        elseif type == "n" then
            return value 
        ...
        --TODO: Complex / Quaternion numbers
        return value
    end,
